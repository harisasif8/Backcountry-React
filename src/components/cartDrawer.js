...
const CartDrawer = ({ isDrawerOpen, onDrawerClose }) => {
    ...
    // useEffect(() => {
    //     // const filteredCartItem = products.find((productId) => parsedCartItem.id === productId.id)
        ...
        >
            ...
            <Drawer.Body >
                ...
                {parsedCartItem.map((cartProductId, index) => {
                    ...
                    const { title: productTitle, productMainImage: { name: BrandName }, activePrice: { maxListPrice: productPrice }, productMainImage: { mediumImg: productImage } } = filteredCartId;
